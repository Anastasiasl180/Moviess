...
import com.example.moviess.di.UserFromGoogle
...
class SignInByGoogle @Inject constructor(
    private val oneTapClient: SignInClient,
    private val auth: FirebaseAuth,
    private val context: Context
){
    suspend fun signInByGoogle(): IntentSender? {
        val result = try {
    ...
    fun getSignedInUser(): UserFromGoogle? = auth.currentUser?.run {
        UserFromGoogle(
            userId = uid,
            username = displayName,
            profilePictureUrl = photoUrl?.toString()
